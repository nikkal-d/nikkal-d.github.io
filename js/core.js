...
export async function exportFlipbook() {
  ...
  for (let i = 0; i < images.length; i += 2) {
    const isCover = (i === 0);
    // Αρχικό z-index για σωστή στοίβαξη
    const zIndex = Math.floor((images.length - i) / 2) + 20;
    leavesHtml += `
      <div class="leaf ${isCover ? 'hard-cover' : ''}" style="z-index: ${zIndex}">
        ...
        <div class="page back">
          ...
          <div class="page-edge-shadow" style="right:0; left:auto; background:linear-gradient(to left, rgba(0,0,0,0.1), transparent)"></div>
        </div>
      </div>`;
  }
  ...
  <head>
    ...
    <style>
      ...
      .btn { padding:10px 16px; border:none; border-radius:20px; cursor:pointer; font-weight:bold; color:white; background: #444; font-size:11px; transition: 0.2s; }
      ...
      .viewport { flex:1; width:100%; overflow: auto; display: grid; place-items: center; }
      ...
      /* Βιβλίο με σωστό Perspective για ρεαλισμό */
      .book { 
      ...
      /* Σκιά στη ραφή που δεν "διπλώνει άσχημα" */
      .page-edge-shadow { 
    ...
    <script>
      ...
      function n() {
        if (cur < leafs.length) {
          ...
          // Δυναμική αλλαγή z-index για να μη "χτυπάει" στις επόμενες
          setTimeout(() => { leaf.style.zIndex = cur; }, 500);
          cur++; updatePos();
        }
      }
      ...
      function p() {
        if (cur > 0) {
          ...
          // Επαναφορά z-index στην κορυφή καθώς γυρίζει πίσω
      ...
      function saveAsHtml() {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'Photobook.html'; a.click();
      }
      ...
      async function exportToLink() {
        ...
        try {
          ...
          if (data.success) prompt("Το Link σας (24h):", data.link);
          else throw new Error();
        } catch (e) { alert("Το αρχείο είναι μεγάλο για Link, κατεβάστε το HTML."); }
        btn.innerText = "🔗 LINK";
      }
    </script>
  </body>
